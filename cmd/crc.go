...
var (
	CRC = cli.Command{
		Name:        "crc",
		Usage:       "demo crc <option>",
		Description: "run demo crc",
		Subcommands: []cli.Command{
			subCmdCrc,
		},
	}

	subCmdCrc = cli.Command{
		Name:        "run",
		Usage:       "demo for crc",
		Description: "run demo for crc",
		Action:      runCRC,
	}
)

func runCRC(c *cli.Context) error {
	data := []byte{0x01, 0x03, 0x17, 0x00, 0x49, 0x50, 0x41, 0x64, 0x64, 0x72, 0x65, 0x73, 0x73, 0x3d, 0x31, 0x39, 0x32, 0x2e, 0x31, 0x36, 0x38, 0x2e, 0x31, 0x2e, 0x32, 0x30, 0x32}
	//expected crc16 as 0x37cd
	checksum := CRC16_IBM(data, len(data))

	fmt.Printf("checksum: 0x%04x\n", checksum)
	return nil
}

func CRC16_IBM(data []byte, length int) uint16 {
	crc := uint16(0)

	for _, value := range data {
		index := uint16(value)
		crc = (crc >> 8) ^ CRC16IBMTable[(crc^index)&0xFF]
	}
...
//CRC-16/IBM 	x16+x15+x2+1 	16 		0x8005 	0000 	0000 		true 		true
var CRC16IBMTable = []uint16{
